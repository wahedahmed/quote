...
(function () {
  ...
  async function supaSelect(filters={}) {
    ...
    if (filters.eq_month)  { url.searchParams.set('date', `gte.${filters.eq_month}-01`); url.searchParams.append('date', `lt.${nextMonth(filters.eq_month)}-01`); }
  ...
  async function supaUpdate(id, obj){
    ...
    url.searchParams.set('tenant', `eq.${window.TENANT}`);
    const res = await fetch(url, { method: 'PATCH', headers: hdr(), body: JSON.stringify(obj) });
